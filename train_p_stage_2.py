...
def get_test_args():
    ...
    parser.add_argument("--concept", type=str, default="adrien_brody")
    parser.add_argument("--task_name", type=str, default="test_train_s2")
    
    parser.add_argument("--pre_trained_ckpt_name", type=str, default="test_train_s1")
...
def update_tokens_load_from_pretrained(args,
    ...
    with torch.no_grad():
        ...
        if os.path.exists(ckpt_embed_path):
            ...
            model.showo.get_input_embeddings().weight.data[original_text_vocab_size:original_text_vocab_size + 1 + nums_new_token_i_stage_1] = ckpt_embed_weight.to(model.showo.get_input_embeddings().weight.device)
            if need_init and get_concept_info(concept)[2] == "human" and nums_new_token_i_stage_2 > 0:
        ...
        if model.showo.lm_head.weight.data.shape[0] == new_total_vocab:
            ...
            if os.path.exists(ckpt_lm_head_weight_path):
                ...
                new_lm_head.weight.data[original_text_vocab_size:original_text_vocab_size + 1 + nums_new_token_i_stage_1] = ckpt_lm_head_weight.to(new_lm_head.weight.device)
            else:
            ...
            if hasattr(lm_head, 'bias'):
                ...
                if os.path.exists(ckpt_lm_head_bias_path):
                    ...
                    new_lm_head.bias.data[original_text_vocab_size:original_text_vocab_size + 1 + nums_new_token_i_stage_1] = ckpt_lm_head_bias.to(new_lm_head.weight.device)
                else:
...
def main():
    ...
    for epoch in range(args.epoch):
        ...
        for batch, batch_idx, dataloader_idx in tqdm(combined_dataloader_list):
            ...
            to_be_l2_token_ids = sks_token_id
            l2_reg_embed = torch.sum((model.showo.get_input_embeddings().weight[to_be_l2_token_ids] - orig_embeds[to_be_l2_token_ids]) ** 2)
            l2_reg_lm_weight = torch.sum((model.showo.lm_head.weight[to_be_l2_token_ids] - orig_lm_head_weight[to_be_l2_token_ids]) ** 2)
            l2_reg_lm_bias = torch.sum((model.showo.lm_head.bias[to_be_l2_token_ids] - orig_lm_head_bias[to_be_l2_token_ids]) ** 2)
