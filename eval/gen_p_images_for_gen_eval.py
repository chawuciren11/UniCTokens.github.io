...
def main():
    ...
    t2i_conditions_file = "prompts_to_eval.json"
    with open(t2i_conditions_file, "r") as f:
        all_t2i_conditions = json.load(f)
        
    concept_info_file = os.path.join(data_root, "concept/train", concept, "info.json")
    with open(concept_info_file, "r") as f:
        concept_info = json.load(f)
    generation_type = concept_info["generation_type"]
    t2i_conditions = all_t2i_conditions[generation_type]
    ...
    for t2i_condition in t2i_conditions:
        ...
        if args.inverse_prompt:
            condition = condition.replace("<sks>", adj_tokens_str)
